...
function getDecorationSet(state: EditorState): DecorationSetWithIntervals {
  ...
  const headings: { text: string; level: number; headingLineNumber: number; headingPos: number }[] = [];
    ...
    scheduleIndentEmbedsAfterRender() {
      this.view.requestMeasure({
        ...
        write: (measure: any, view: EditorView) => {
          ...
          const embeds = Array.from(dom.querySelectorAll("div.cm-content > div.cm-embed-block")) as HTMLDivElement[];
          const mathBlocks = Array.from(dom.querySelectorAll("div.cm-content > div.math-block")) as HTMLDivElement[];
          ...
          const imgs = Array.from(dom.querySelectorAll("div.cm-content > img")) as HTMLImageElement[];
          prevIntervalIndex = 0;
 ...
 */
function findIntervalOffset(intervals: [number, number][], pos: number, start: number): [number, number] {
